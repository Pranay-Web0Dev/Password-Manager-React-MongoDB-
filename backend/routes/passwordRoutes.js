...
const { checkPasswordStrength, generatePassword } = require('../utils/validation');
const { generateEncryptionKey, encrypt, decrypt } = require('../utils/encryption');
const { sendPasswordAccessedEmail, sendPasswordUpdatedEmail, sendFailedLoginEmail, sendOTPEmail } = require('../utils/emailService');
const { getClientIp } = require('../utils/helpers');

// All routes require authentication
router.use(protect);

// Helper function to check and handle account lock
const checkAccountLock = async (user, res) => {
  if (user.isLocked()) {
...
router.post('/:id/decrypt', async (req, res) => {
  try {
    ...
    // Get user to check lock status and track attempts
    const user = await User.findById(req.user.id);
    if (!user) {
      return res.status(404).json({ 
        success: false, 
    ...
    const lockResponse = await checkAccountLock(user, res);
